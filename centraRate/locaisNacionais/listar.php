...
<div class="container tema">
    <div class="row">
		...
		for ($i = 0; $i <= 150; $i++) {
         	if($ramal[$i] != ''){
         	$ramal1 = $ramal[$i];
                    $sql = "select src, dst, billsec, calldate from cdr where dst > 6000 and '$ramal1' = src and (select substring(dst,1,1)) < 5 and (select length(dst) as numero2) = 8 and calldate >= '$dataInicio 00:00:00' AND calldate <= '$dataFinal 23:59:59' AND disposition = 'ANSWERED';";
...
<div class="container tema">
    <div class="row">
		...
		for ($i = 0; $i <= 150; $i++) {
                if($ramal[$i] != ''){
                $ramal1 = $ramal[$i];
                    $sql1 = "select src, dst, billsec, calldate from cdr where dst > 6000 and '$ramal1' = src and (select substring(dst,3,1)) BETWEEN 1 AND 5 and (select length(dst) as numero2) = 10 and calldate >= '$dataInicio 00:00:00' AND calldate <= '$dataFinal 23:59:59' AND disposition = 'ANSWERED' UNION select src, dst, billsec, calldate from cdr where dst > 6000 and '$ramal1' = src and (select substring(dst,4,1)) BETWEEN 1 AND 5 and (select length(dst) as numero2) = 11 and calldate >= '$dataInicio 00:00:00' AND calldate <= '$dataFinal 23:59:59' AND disposition = 'ANSWERED' UNION select src, dst, billsec, calldate from cdr where dst > 6000 and '$ramal1' = src and (select substring(dst,6,1)) BETWEEN 1 AND 5 and (select length(dst) as numero2) = 13 and calldate >= '$dataInicio 00:00:00' AND calldate <= '$dataFinal 23:59:59' AND disposition = 'ANSWERED';";
...
//                    $cont_masculino = 0;
//                    $cont_nao_infor = 0;
                    while($linha2 = mysql_fetch_assoc($resultado1))
                    {
                        echo '<tr>';
//                            echo '<td>'. $linha['id'] .'</td>';
                            echo '<td style=width:15%>'. $linha2['src'] .'</td>';
                            echo '<td style=width:30%>'. $linha2['dst'] .'</td>';
                            echo '<td style=width:30%>'. $linha2['calldate'] .'</td>';
                            echo '<td style=width:10%>'. $linha2['billsec'] .'</td>';
                            $ligacao2=($linha2['billsec'] /60) * 0.25;
                            $ligacao2 = number_format($ligacao2, 2, ',', '');;
                            echo "<td align=center style='width:20%;'>". $ligacao2 ."</td>";
